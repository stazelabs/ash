...
- **No `make validate` run yet (gated on `ANTHROPIC_API_KEY`).** This ticket changes the wire shape, not substitution rules, so `validate-check` is the load-bearing gate — but if a future session has the key, running `make validate` will confirm the cross-tokenizer agreement on the new envelope shape.
