...
For the *MCP tool surface* — JSON Schema draft 2020-12 tool definitions derived from the same `help.Registry()` — [docs/mcp/tools.json](docs/mcp/tools.json) is the checked-in artifact (ASH-105). Regenerate with `make schema` after any verb-schema edit (same trigger as `make vocab`); `make schema-check` is the CI drift gate. ASH-104 (`ashmcp`) embeds this file via `//go:embed`.
