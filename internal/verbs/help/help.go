...
var registry = []VerbSchema{
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "in", Type: "string", Default: "", PH: "<p>", Ops: []string{"def", "refs", "callers", "impl"},
				Description: "Optional file scope — disambiguates the symbol/interface lookup to one file.",
				Long:        "When set, the symbol-name resolution (workspace/symbol) is filtered to this file. Useful when a name is overloaded across packages. For impl, this disambiguates WHICH interface, not where the implementers live — implementers can live anywhere in the workspace."},
