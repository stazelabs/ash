...
var registry = []VerbSchema{
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "repeat", Type: "int", Default: "1", Description: "Measured iterations per case per side. >1 enables p50 + min latency reporting; tokens are deterministic and pinned to the first measured iteration."},
			{Name: "warmup", Type: "int", Default: "0", Description: "Unmeasured iterations per case per side, run before measurement to prime filesystem cache and runtime state. Defaults to 1 when --repeat>1, 0 otherwise."},
			{Name: "list", Type: "bool", Default: "false", Description: "List recent persisted bench runs and exit (no fresh bench)."},
			{Name: "list_limit", Type: "int", Default: "20", Description: "Cap the --list output."},
			{Name: "compare", Type: "string", Default: "", PH: "<A,B>", Description: "Compare two persisted runs by uuid (prefix accepted). Tokens 'latest' resolves to the most recent run. Output is a side-by-side per-case delta with REGRESS / IMPROVE flags."},
			{Name: "baseline", Type: "string", Default: "", PH: "<dur>", Description: "Run a fresh bench, then compare against the per-case rolling median over the given window (e.g. 7d, 24h). Persists the fresh run."},
			{Name: "regress_tokens", Type: "int", Default: "10", Description: "Δtok%% threshold above which a case counts as a regression in --compare output."},
			{Name: "regress_latency", Type: "int", Default: "20", Description: "Δlat%% threshold above which a case counts as a regression."},
			{Name: "record_baseline", Type: "bool", Default: "false", Description: "Run a fresh bench, then write bench/baseline.json + bench/baseline.md + bench/latency-snapshot.json. The baseline.json file is the regression contract checked into the repo."},
			{Name: "export_md", Type: "bool", Default: "false", Description: "Render the latest persisted bench run as Markdown. Pipe stdout into bench/baseline.md."},
		},
	},
	{
		Verb:        "test",
