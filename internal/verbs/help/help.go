...
var registry = []VerbSchema{
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "old", Type: "string", Mode: "string", PH: "<text|->",
				Description: "Exact text to find; pass '-' to read from stdin; must appear once unless all=true.",
				Long:        "[string mode] Exact text to find (required if range/patch not provided). Must appear exactly once unless all=true. Pass '-' to read from stdin — useful for multi-line patterns with shell-tricky characters (backticks, braces, nested quotes)."},
			{Name: "new", Type: "string", Mode: "string", Default: "", PH: "<text|->",
				Description: "Replacement text; pass '-' to read from stdin; empty string deletes the match.",
				Long:        "[string mode] Replacement text. Empty string deletes the matched text. Pass '-' to read from stdin."},
			...
			{Name: "new", Type: "string", Mode: "range", Default: "", PH: "<text|->",
				Description: "Replacement lines; pass '-' to read from stdin; empty string deletes the range.",
				Long:        "[range mode] Replacement text for the specified lines. Empty string deletes the lines. Pass '-' to read from stdin."},
