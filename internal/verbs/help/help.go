...
var registry = []VerbSchema{
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "path", Type: "string", Default: ".",
				...
				Long:        "File or directory to search; defaults to \".\" (the walk root) when omitted. Match paths in results are repo-root-relative by default (ASH-71); pass --absolute true for the legacy input-mirroring form."},
			{Name: "glob", Type: "string", Default: "**",
			...
			{Name: "limit", Type: "int", Default: "256",
				Description: "Cap on total match records; hard cap 4096.",
	...
	},
	{
		Verb:        "git",
		Tier:        "A",
		Description: "Structured git calls via --op discriminator: status, log, diff, show, blame.",
		Args: []ArgSchema{
			{Name: "op", Type: "string", Required: true, Values: []string{"status", "log", "diff", "show", "blame"},
				Description: "Subcommand to run."},
			{Name: "path", Type: "string", Default: ".",
				Description: "Any path inside the git worktree; output is repo-root-relative.",
				Long:        "Repository path (any path inside a git work tree). Note: returned file paths are always repo-root-relative regardless of how --path was passed. This departs from find/grep where paths mirror the --path form. [blame] --path must point at a single file, not a directory."},
			...
			{Name: "rev", Type: "string", Default: "HEAD", PH: "<rev>", Ops: []string{"blame"},
				Description: "Revision to blame at; defaults to HEAD.",
				Long:        "[blame] revision to blame the file at (SHA, HEAD, HEAD~1, branch, tag). Defaults to HEAD."},
			{Name: "lines", Type: "string", Default: "", PH: "<range>", Ops: []string{"blame"},
				Description: "1-based inclusive line range start:end; open endpoints allowed.",
				Long:        "[blame] restrict blame to a line range (1-based inclusive). Examples: '100:200' lines 100 through 200; '100:' from line 100 to end; ':200' from line 1 to 200. Out-of-bounds start returns range_out_of_bounds; out-of-bounds end clamps to file length."},
			...
			{Name: "bytes", Type: "int", Default: "262144", Ops: []string{"diff", "show", "blame"},
				Description: "Max output bytes; truncates trailing content past cap.",
				Long:        "[diff/show] cap on total patch bytes returned. Files beyond cap have patch omitted but stats preserved. [blame] cap on serialized hunk bytes; trailing hunks are dropped past the cap and TruncInfo carries the count. Max 4 MiB across all three ops."},
		},
	},
	{
		Verb:        "write",
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "since", Type: "string", Default: "",
				Description: "Time window (e.g. 15m, 1h, 7d, 1w, 2mo); Go duration + d/w/mo.",
				Long:        "Time window, e.g. '15m', '1h', '24h', '7d', '1w', '2mo'. Supports Go duration syntax plus d=24h, w=7d, mo=30d (calendar-approximate)."},
	...
	},
	{
		...
		Args: []ArgSchema{
			{Name: "since", Type: "string", Default: "1h",
				Description: "Time window (e.g. 15m, 1h, 24h, 7d, 1w); Go duration + d/w/mo.",
				Long:        "Time window over which to aggregate the session. Supports Go duration syntax plus d=24h, w=7d, mo=30d (calendar-approximate). Hard cap 7d — anything longer is capped silently."},
	...
	},
	{
		...
		Args: []ArgSchema{
			{Name: "since", Type: "string", Default: "30m",
				Description: "Time window for relevance, default 30m; max 24h. Go duration + d/w/mo.",
				Long:        "Time window used to gather recently-touched files and recent searches. Supports Go duration syntax plus d=24h, w=7d, mo=30d (calendar-approximate). Hard cap 24h."},
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "since", Type: "string", Default: "",
				Description: "Time window (e.g. 1h, 7d, 1w, 2mo); Go duration + d/w/mo.",
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "fuzz", Type: "int", Mode: "patch", Default: "3",
				Description: "Relax hunk placement: search ±N lines for matching context; 0 = strict.",
				Long:        "[patch mode] When the hunk header's authored line doesn't match the source, scan ±N lines outward for the first matching anchor (first context/delete line in the hunk body). Default 3 mirrors GNU patch --fuzz=3. Set to 0 to require the authored line number to match exactly. Hard cap 10. Per-line context matching within the hunk body remains strict regardless of fuzz — only placement is relaxed (ASH-152). Result.fuzz_applied counts hunks placed via the scan."},
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "micro", Type: "bool", Default: "false",
				Description: "Run Go Benchmark* functions for curated hot-path packages.",
				Long:        "Run 'go test -bench=. -benchmem' over cmd/ash, internal/verbs/hook, internal/verbs/grep, internal/walker, and internal/gitignore. Returns ns/op, B/op, allocs/op per sub-case."},
			{Name: "record_micro", Type: "bool", Default: "false",
				Description: "Run micro bench and write bench/microbench.json + bench/microbench.md.",
				Long:        "Run --micro, then write bench/microbench.json and bench/microbench.md. Use 'ash bench --diff-micro' to compare a future run against this saved baseline."},
			{Name: "diff_micro", Type: "bool", Default: "false",
				Description: "Run micro bench and compare against bench/microbench.json.",
				Long:        "Run --micro and compare ns/op per sub-case against bench/microbench.json. Flags regressions exceeding --regress-latency (default 20%). Requires a prior --record-micro run."},
			{Name: "micro_benchtime", Type: "string", Default: "1s",
				Description: "go test -benchtime for --micro mode (e.g. '1s', '3s')."},
			{Name: "micro_count", Type: "int", Default: "1",
				Description: "go test -count for --micro mode; 1 runs each benchmark once per benchtime window."},
			{Name: "micro_packages", Type: "string", Default: "", PH: "<pkgs>",
				Description: "Comma-separated package paths for --micro; default: curated hot-path set.",
				Long:        "Override the curated package list. Comma-separated paths, e.g. 'cmd/ash,internal/walker'. Packages without Benchmark* functions are silently skipped."},
		},
	},
	{
		Verb:        "build",
	...
	},
	{
		...
		Args: []ArgSchema{
			...
			{Name: "cover", Type: "bool", Default: "false",
				Description: "go test -cover; per-pkg coverage % surfaced on Package.Coverage.",
				Long:        "When true, append -cover to the go test invocation and parse the per-package 'coverage: X.X% of statements' line. Passing packages with a numeric percent get Package.Coverage set; packages with no executable statements (the '[no statements]' form) stay nil."},
		},
	},
	{
		Verb:        "init",
		...
		Args: []ArgSchema{
			...
			{Name: "refresh", Type: "bool", Default: "false",
				Description: "Narrowly refresh only the ash-managed CLAUDE.md section.",
				Long:        "Rewrite only the marker-bracketed ash-managed section in CLAUDE.md/AGENTS.md when its content has drifted from the current template. Skips settings.json, .gitignore, and the registry. No-op if the section already matches; warns (no write) if the file is missing or has no ash markers. Safe to run unattended; the install/upgrade path that does not require --force."},
	...
	},
	{
		Verb:        "usage",
		Tier:        "D",
		Description: "Estimate cache-friendliness of recent calls from arg-repetition counts.",
		Args: []ArgSchema{
			{Name: "since", Type: "string", Default: "24h", PH: "<dur>",
				Description: "Time window; Go duration + d/w/mo.",
				Long:        "Restricts the aggregation to calls in this window. Accepts Go duration syntax plus the extended d/w/mo suffixes (e.g. 30m, 24h, 7d, 1w, 1mo)."},
			{Name: "session", Type: "string", Default: "current",
				Description: "Session scope: current, all, or explicit session ID.",
				Long:        "Restricts the aggregation to a specific session. \"current\" (default) is the active daemon session; \"all\" aggregates across every session in the ledger; an explicit 8-byte session id narrows to that one."},
		},
	},
	{
		Verb:        "turn",
	...
	},
	{
		...
		Args: []ArgSchema{
			{Name: "op", Type: "string", Required: true, Values: []string{"outline", "def", "refs", "callers", "impl"},
				Description: "Subcommand to run.",
				Long:        "Operation discriminator. outline: per-file symbols. def: definition site. refs: every reference. callers: incoming call graph (1 level). impl: implementations of an interface."},
			...
			{Name: "symbol", Type: "string", Default: "", PH: "<name>", Ops: []string{"def", "refs", "callers"},
				Description: "Symbol name to find; required for def/refs/callers.",
				Long:        "Exact symbol name. workspace/symbol drives the lookup; substring/fuzzy matches are dropped. For refs/callers, the position is resolved internally before the position-based LSP call."},
			{Name: "interface", Type: "string", Default: "", PH: "<name>", Ops: []string{"impl"},
				Description: "Interface name; required for op=impl.",
				Long:        "[impl] interface name. Same exact-name lookup as --symbol; emits one row per implementing type."},
			{Name: "in", Type: "string", Default: "", PH: "<p[,p…]>", Ops: []string{"def", "refs", "callers", "impl"},
				Description: "Optional file/directory scope (comma-separated) — narrow which candidates the symbol/interface resolution considers.",
				Long:        "When set, candidates whose path lies under one of the listed prefixes are kept; others are dropped. Multi-path form (ASH-158): pass \"a,b,c\" to allow a candidate under any of a/, b/, or c/. For impl, this narrows WHICH interface, not where the implementers live — implementers can live anywhere in the workspace."},
			{Name: "internal-only", Type: "bool", Default: "true", Ops: []string{"def", "refs", "callers", "impl"},
				Description: "Drop candidates from $GOROOT and module cache; pass false to include them.",
				Long:        "[ASH-158] Filter out symbols whose path lies under $GOROOT or $GOPATH/pkg/mod (default ~/go/pkg/mod). The agent usually means \"in this repo\" when they ask for a symbol; pass --internal-only false to opt back into stdlib + dep matches."},
			{Name: "limit", Type: "int", Default: "256", Ops: []string{"refs", "callers", "impl"},
				Description: "Cap on rows returned; hard cap 4096.",
				Long:        "[refs/callers/impl] cap on returned rows. Default 256; hard cap 4096. Excess rows are dropped silently — narrow with --in or a more specific name if you hit it."},
			{Name: "context", Type: "bool", Default: "true", Ops: []string{"refs", "callers", "impl"},
				Description: "Include the source line at each match.",
...
func PrettyResponse(req *proto.Request, rsp *proto.Response) string {
	...
	if r.Count != 1 {
		fmt.Fprintf(&b, "§help: %d verb(s) — use `ash help <verb>` for full schema\n", r.Count)
