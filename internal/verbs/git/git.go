...
//
// Op discriminator (today: only "status"):
//
//	status   summarize repo state — branch, ahead/behind, staged/unstaged/
//	         untracked/conflicts. Optional: --untracked, --ignored.
//
// Implementation shells out to system `git` and parses the documented
// `--porcelain=v2` output. The agent never sees porcelain; they get a
// typed Result. We don't use go-git for now because shelling out is the
// shortest correct path and `git` is universal on developer machines.
//
...
// Run dispatches by op. Single op for now; the structure makes adding
// log/diff/blame a copy-and-extend exercise.
func Run(a *Args, tr *proto.Tracer) (*Result, *proto.Error) {
	...
	default:
		return nil, &proto.Error{Code: "unknown_op", Msg: "unknown op: " + a.Op + " (live ops: status)"}
	}
}
...
func runStatus(a *Args, tr *proto.Tracer) (*StatusResult, *proto.Error) {
	...
	if runErr != nil {
		...
		if errors.As(runErr, &exitErr) {
			...
			// Map "not a git repository" to a typed error code so the agent
			// can react cleanly without parsing stderr.
...
// PrettyResponse renders the result by op. Today only status; future ops
// dispatch off the request's op flag.
